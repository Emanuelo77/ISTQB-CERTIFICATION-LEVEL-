...
// Funcție pentru amestecarea întrebărilor
...
// Funcție pentru a verifica răspunsul și a avansa la următoarea întrebare
function checkAnswer() {
    const selectedAnswerIndex = document.querySelector('input[name="answer"]:checked');
    if (selectedAnswerIndex) {
        const isCorrect = questions[currentQuestionIndex].answers[selectedAnswerIndex.value].correct;
        if (isCorrect) score++;
    }
    currentQuestionIndex++;
    if (currentQuestionIndex < 40 && currentQuestionIndex < questions.length) {
...
// Funcție pentru finalizarea quiz-ului
function endQuiz() {
    clearInterval(timer);
    ...
    let smiley = '';
    if (percentage >= 65) {
        resultMessage = `Bestanden! Sie haben ${percentage}% erreicht.`;
        smiley = '😊';
    } else {
        resultMessage = `Nicht bestanden! Sie haben nur ${percentage}% erreicht.`;
        smiley = '😢';
    }
...
// Event Listeners pentru butoanele Next și Prev
...
// Adăugare Event Listener pentru butonul "Prüfung beenden"
